...
if __name__ == '__main__':
	...
	#send request
	request = input("Please enter a file or a equation: ")
	...
	if result == request:
		...
		with open('copy'+filename, 'wb') as f:
			while True:
				...
				recv_packet.separate(raw_packet)
				print("Receive a packet (seq_num = %d ack_num = %d)"%(recv_packet.seq_num()/MSS,recv_packet.ack_num()))
				print("expected_seq_num:",expected_seq_num)
				print("recv_packet.seq_num():",recv_packet.seq_num())
